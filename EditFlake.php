...
if ((isset($MM_update)) && ($MM_update == "editform") && ($File_is_Uploaded == TRUE)) {
    ...
    $oldImage = snowflakeStruct::getImageNameById($SFconnects, $post_Id);
...
$colname_rsUpdatepost = -1;
$pageid = filter_input(INPUT_GET, 'pageid');
if (isset($pageid)) {
    $colname_rsUpdatepost = $pageid;
}
$snowflakeStruct = new snowflakeStruct();
$snowflakeStruct->getSnowflakesByid($SFconnects, $colname_rsUpdatepost);
    ...
    <body> 
                ...
                <div class="PageWrap">
                    <?php if (isset($pageid)) { ?>
                        ...
                        if (!empty($formmessage)) {
                            ...
                            echo sfUtils::dialogMessage("Edit Snowflakes", $Message);
                        }
                        ?>
                        <!--contactform-->
                        <div class="contactform">
                            <form method="post" action="<?php echo $editFormAction; ?>" enctype="multipart/form-data" name="editform" id="installForm">
                                <span id="sprytTitle">
                                    ...
                                    <input class="inputtext2 controls" type="text" name="title" value="<?php echo $snowflakeStruct->m_title; ?>" />
                                </span><br />
                                ...
                                <span id="spryBodyText">
                                    ...
                                    <iframe name="richTextField" id="richTextField sfeditor_textbox"style="min-height:300px;"  class="inputtext2 controls"></iframe><br>
                                    <textarea style="display:none;" id="Pmessage2" class="controls" name="body_text" cols="50" rows="8"><?php echo $snowflakeStruct->m_body_text; ?></textarea>
                                </span>
                                <br />
                                <div class="SnowflakeImageSmall">
                                    <a class="colorbox" href="<?php echo $UploadImgUrl . $snowflakeStruct->m_image_name; ?>" onerror="this.href='<?php echo $imageMissing; ?>'" title="Snowflake Image" > 
                                        <img src="<?php echo $UploadImgUrl . $snowflakeStruct->m_image_name; ?>" onerror="this.src='<?php echo $imageMissing; ?>'" alt="Event Image" />
                                    </a>
                                </div>
                                ...
                                <div class="publishswitch">
                                    <input type="checkbox" name="publish" class="publishswitch-checkbox" id="mypublishswitch" <?php if (!(strcmp($snowflakeStruct->m_publish, 1))) echo "checked"; ?>>
                                ...
                                <select name="gallery" class="inputtext3 controls">
                                    <option value="" <?php
                                    if (!(strcmp("", $snowflakeStruct->m_gallery))) {
                                        echo "selected=\"selected\"";
                                    }
                                    ?>>Add a Galley</option>
                                            <?php
                                            $GalleryName = explode(",", $snowflakeStruct->m_gallery);
                                            $i = 0;
                                            do {
                                                ?>
                                        <option value="<?php echo $row_rsGallery[$i]['id'] . "," . $row_rsGallery[$i]['title']; ?>"<?php
                                        if (!(strcmp($row_rsGallery[$i]['id'], $GalleryName[0]))) {
                                            echo "selected=\"selected\"";
                                        }
                                        ?>><?php echo $row_rsGallery[$i]['title'] ?></option>
                                                <?php
                                                $i++;
                                ...
                                <input class="NewButton" type="submit" value="Change Flake" onClick="transferEditorData('editform');" />
                                <input type="hidden" name="image_name" value="<?php
                                if (empty($uploadedFile)) {
                                    $uploadedFile = $snowflakeStruct->m_image_name;
                                    echo $uploadedFile;
                                } else {
                                    echo $uploadedFile;
                                }
                                ...
                                <input name="id" type="hidden" id="pageid" value="<?php echo $snowflakeStruct->m_id; ?>" />
        ...
        <?php if (isset($pageid)) { ?>
            <script type="text/javascript">
                ...
                var sprytextarea1 = new Spry.Widget.ValidationTextarea("spryBodyText", {validateOn: ["blur"]});
            </script>
        <?php } ?>
        <!-- InstanceEndEditable -->
    </body>
