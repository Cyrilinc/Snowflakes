	...
	*/
	...
	$.PFold.prototype = {
		...
		_init : function( options ) {
			...
			// initial content
			this.$iContent = this.$el.find( '.SFEvent-initial' );
			this.iContent = this.$iContent.html();
			// final content
			this.$fContent = this.$el.find( '.SFEvent-final' );
			...
			this.$finalEl = $( '<div class="SFEvent-final-wrapper"></div>' ).append( this.$iContent.clone().hide(), this.$fContent ).hide();
		...
		// adds wrappers to the SFEvent-initial-content and SFEvent-final-content divs
		_layout : function() {

			var $initialContentEl = this.$el.children( 'div.SFEvent-initial-content' ),
				finalDim = this._getFinalDim(),
				$finalContentEl = this.$el.children( 'div.SFEvent-final-content' ).css( {
					width : finalDim.width,
					height : finalDim.height
				} );

			$initialContentEl.wrap( '<div class="SFEvent-initial"></div>' );
			$finalContentEl.show().wrap( $( '<div class="SFEvent-final"></div>' ) );
		...
		_start : function( action, step ) {
			...
			// remove SFEvent-part divs inside the container (the top and bottom elements)
			this.$el.find( 'div.SFEvent-part' ).remove();
			...
			switch( step ) {
				...
				case 0 : case this.options.folds - 1 :
					...
					if( action === 'fold' ) {
						...
						if( step === 0 ) {
							...
							this.$el.addClass( 'SFEvent-current' ).css( { left : coords.ftx, top : coords.fty } );
				...
				case this.options.folds :
					...
					if( action === 'fold' ) {
						...
						this.$el.removeClass( 'SFEvent-current' );
			...
			var unfoldClass = 'SFEvent-unfold-' + direction,
				topElClasses = ( action === 'fold' ) ? 'SFEvent-unfold SFEvent-part ' + unfoldClass : 'SFEvent-part ' + unfoldClass,
				$topEl = $( '<div class="' + topElClasses + '"><div class="SFEvent-front">' + contentTopFront + '</div><div class="SFEvent-back">' + contentTopBack + '</div></div>' ).css( styleCSS ),
				$bottomEl = $( '<div class="SFEvent-part SFEvent-single">' + contentBottom + '</div>' ).css( styleCSS );
			...
			setTimeout( function() {
				...
				// apply style
				( action === 'fold' ) ? $topEl.removeClass( 'SFEvent-unfold' ) : $topEl.addClass( 'SFEvent-unfold' );

				if( self.support ) {

					$topEl.on( self.transEndEventName , function(event) {

						if( event.target.className !== 'SFEvent-flipoverlay' && step < self.options.folds ) {
		...
		_setLastStep : function( direction, styleCSS ) {
			...
			contentBottom = '<div class="SFEvent-inner"><div class="SFEvent-inner-content" style="' + contentBottomStyle + '">' + this.fContent + '</div></div>';

			var contentTopBackClasses = direction === 'top' || direction === 'bottom' ? 'SFEvent-inner SFEvent-inner-rotate' : 'SFEvent-inner';
				contentTopBack = '<div class="' + contentTopBackClasses + '"><div class="SFEvent-inner-content" style="' + contentTopBackStyle + '">' + this.fContent + '</div></div>';
		...
		_addOverlays : function( action, $bottomEl, $topEl ) {
			...
			this.$bottomOverlay = $( '<div class="SFEvent-overlay"></div>' );
			this.$topFrontOverlay = $( '<div class="SFEvent-flipoverlay"></div>' );
			this.$topBackOverlay = $( '<div class="SFEvent-flipoverlay"></div>' );
			...
			$topEl.children( 'div.SFEvent-front' )
				  .append( this.$topFrontOverlay.css( topFrontOverlayStyle ) )
				  .end()
				  .children( 'div.SFEvent-back' )
